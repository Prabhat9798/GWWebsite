...
const TestiCard = ({ item }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden transform transition duration-300 hover:-translate-y-1 hover:shadow-md mx-1 flex flex-col h-full min-h-[380px] sm:min-h-[400px]">
      <div className="bg-gradient-to-r from-orange-100 to-red-100 p-4 pt-14 sm:pt-16 relative">
        <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
          <img
            src={item.imageSrc}
            alt={item.name}
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-white shadow-md object-cover"
          />
        </div>

        <div className="text-gray-700 text-sm sm:text-md leading-relaxed min-h-[80px] mt-12">
          <p>{item.para1}</p>
          <p>{item.para2}</p>
        </div>
      </div>
      ...
      <div className="px-4 py-3 flex-1">
        <h3 className="font-bold text-md sm:text-lg text-gray-900">{item.name}</h3>
      ...
      <div
        className="bg-gradient-to-r from-orange-500 to-red-500 text-white py-3 sm:py-4 flex items-center justify-center gap-2 cursor-pointer hover:opacity-90 text-sm"
...
function Testimonal() {
  ...
  return (
    <div className="mb-4 py-6 sm:py-8">
      <div className="flex flex-col items-center mb-4 sm:mb-6">
        <div className="w-full max-w-6xl px-4 relative">
          <div className="flex justify-between items-center">
            <div className="flex-grow text-center">
              <h3 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-[#D63715] to-[#FF9422] bg-clip-text text-transparent inline-block">
